...
SELECT
    ...
    CASE
        WHEN (SELECT TRIM(APPEND_TRANS) FROM TAB_SCHOOL WHERE TRIM(MINCODE) = (SELECT dist.DISTRICT_NUMBER || school.SCHOOL_NUMBER from DISTRICT dist WHERE dist.DISTRICT_ID = school.DISTRICT_ID)) = 'Y' THEN 'APPEND'
        ELSE 'REPLACE'
